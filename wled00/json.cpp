...
void deserializeSegment(JsonObject elem, byte it, byte presetId)
{
  ...
  if (elem["frz"].is<const char *>() && elem["frz"].as<const char *>()[0] == 't')
    frz = !seg.getOption(SEG_OPTION_FREEZE);
  ...
  if (!colarr.isNull())
  {
    for (uint8_t i = 0; i < 3; i++)
    {
      ...
      if (colX.isNull())
      {
        ...
        if (hexCol == nullptr)
        { // Kelvin color temperature (or invalid), e.g 2400
      ...
      { // Array of ints (RGB or RGBW color), e.g. [255,160,0]
        byte sz = colX.size();
        if (sz == 0)
...
// lx parser
  ...
  byte fx = seg.mode;
  if (getVal(elem["fx"], &fx, 1, strip.getModeCount()))
  { // load effect ('r' random, '~' inc/dec, 1-255 exact value)
...
bool deserializeState(JsonObject root, byte callMode, byte presetId)
{
  ...
  if (bri && !onBefore)
  { // unfreeze all segments when turning on
    ...
    { // keep live segment frozen if live
  ...
  int tr = -1;
  if (!presetId || currentPlaylist < 0)
  { // do not apply transition time from preset if playlist active, as it would override playlist transition times
  ...
  tr = root[F("tb")] | -1;
  if (tr >= 0)
    strip.timebase = ((uint32_t)tr) - millis();
  ...
  if (segVar.is<JsonObject>())
  {
    ...
    if (id < 0)
    {
      ...
      if (!didSet)
        deserializeSegment(segVar, strip.getMainSegmentId(), presetId);
    }
    else
    {
  ...
  else
  {
    ...
    ps = presetCycCurr;
    if (getVal(root["ps"], &ps, presetCycMin, presetCycMax))
    { // load preset (clears state request!)
      if (!presetId)
        unloadPlaylist(); // stop playlist if preset changed manually
      if (ps >= presetCycMin && ps <= presetCycMax)
        presetCycCurr = ps;
...
void serializeSegment(JsonObject &root, WS2812FX::Segment &seg, byte id, bool forPreset, bool segmentBounds)
{
  ...
  if (segmentBounds && seg.name != nullptr)
...
void serializeState(JsonObject root, bool forPreset, bool includeBri, bool segmentBounds)
{
  ...
  for (byte s = 0; s < strip.getMaxSegments(); s++)
  {
    ...
    else if (forPreset && segmentBounds)
    { // disable segments not part of preset
      JsonObject seg0 = seg.createNestedObject();
      seg0["stop"] = 0;
    }
  }
...
void serveJson(AsyncWebServerRequest *request)
{
  ...
  case 4: // node list
    ...
    break;
  case 5: // palettes
    serializePalettes(lDoc, request);
    break;
  default: // all
