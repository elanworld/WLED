...
int findEffectIndex(String effect) {
  ...
  for (int i = 0; i < strip.getModeCount(); i++) {
    ...
    if (lineBuffer[0] != 0) {
      ...
      if (dataPtr) *dataPtr = 0;  // keep name only
    }
  ...
  if (strcmp_P(topic, PSTR("/command")) == 0) {
    if (payloadStr.c_str()[0] == '{') {
      ...
      if (commandJson.containsKey("state") &&
          strcmp_P(commandJson["state"], "OFF") == 0) {
        if (bri != 0) {
          briLast = bri;
          bri = 0;
          updated = true;
        }
      } else if (commandJson.containsKey("state") &&
                 strcmp_P(commandJson["state"], "ON") == 0) {
        if (commandJson.containsKey("brightness") &&
            commandJson["brightness"] > 0) {
          briLast = bri;
      ...
      // update fx
      DEBUG_PRINTLN("update fx");
      int effectIndex = findEffectIndex(payloadStr);
      if (effectIndex != -1) {
        for (uint8_t i = 0; i < strip.getSegmentsNum(); i++) {
          strip.setMode(i, effectIndex);
          stateChanged = true;
